...
![image-20240219094216013](C:\Users\wanglin25\AppData\Roaming\Typora\typora-user-images\image-20240219094216013.png)

可以看到，文字字号缩小一倍，耗时由`40s`减至了`2.7s`，效果是非常惊人的。



# 旋转

接下来我们让文字支持旋转，因为我们是基于像素进行计算的，所以文字怎么旋转对于计算过程而言都是无所谓的，唯一要修改的是获取文字像素数据的方法，因为`canvas`的大小目前是根据文字的宽高设置的，当文字旋转后就不行了：
...
const getTextImageData = (text, fontStyle, rotate = 0) => {
    let { width, height } = measureText(text, fontStyle)
    const rect = getRotateBoundingRect(width, height, rotate)
    width = rect.width
    height = rect.height
    // ...
    // 绘制文本
...
class WordItem {
    constructor({ text, weight, fontStyle, color, rotate }) {
        ...
        // ...
        // 文本像素数据
        this.imageData = getTextImageData(text, fontStyle, this.rotate)
        // ...
    }
}
```
